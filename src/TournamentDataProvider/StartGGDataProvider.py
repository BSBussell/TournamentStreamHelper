...
class StartGGDataProvider(TournamentDataProvider):
    ...
    def GetRecentSets(self, id1, id2, videogame, callback, requestTime, progress_callback, cancel_event):
        try:
            ...
            # Fetch both player-orderings so Start.gg's directed recent-sets
            # query does not truncate a head-to-head record. Keep the deeper
            # local history window while using the current Worker result API.
            workers = []
            ...
            Worker.wait_for_all(workers, timeout=30.0)
