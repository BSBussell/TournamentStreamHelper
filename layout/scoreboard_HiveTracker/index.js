...
const useHiveCustomScoreColors = () =>
...
LoadEverything().then(() => {
            ...
            for (const [t, team] of [
                ...
                for (const [p, player] of [team.player["1"]].entries()) {
                    if (player) {
                        ...
                        SetInnerHtml(
                            $(`.p${t + 1} .flagcountry`),
                            player.country.asset && showLocation
                    ...
                    if (
                        team.color &&
                        !tsh_settings["forceDefaultScoreColors"] &&
                        !useHiveCustomScoreColors()
                    ) {
                        document
                            .querySelector(":root")
                            .style.setProperty(
                                `--p${t + 1}-score-bg-color`,
                                team.color,
                            );
                    }
                }
    ...
    function colorInBoxes() {
        for (let i = 0; i < data.score[window.scoreboardNumber].best_of; i++) {
            ...
            const darkGameBox = document.querySelector(
                `.game${i + 1}.neither_won`,
            );
    ...
    if (newP1Score < p1Score) {
        ...
        if (p1Score < 100) {
            for (let i = 0; i < p1Score - newP1Score; i++) {
                ...
                for (
                    let j = 0;
                    j < data.score[window.scoreboardNumber].best_of;
                    j++
                ) {
                    if (gameArray[j] == 1) {
