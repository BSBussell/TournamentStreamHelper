# HiveTracker styling pass

This folder is copied from JinTracker. `index.js`, `index.css`, and settings remain
unchanged; `index.html` adds portrait-frame SVGs and loads `hive.css` last.
The shared logo, fonts, placement, contrast handling, and game-history behavior
are inherited. All new accent colors use the existing P1/P2 score CSS variables.

The hexagonal character surrounds overlap the original panel silhouette. Thin
highlight strokes, small honey cells, and short wax drips decorate its edges.
`noCharacter.html` remains the original unstyled alternative.

Use a 1920×1080 OBS browser source. `preview.png` is a transparent strip capture;
`index_preview.png` shows the overlay over the repository’s gameplay screenshot.
Browser preview used real TSH state; OBS itself was not tested. The caveats below
are inherited from JinTracker; this styling pass does not change its controller.
...
- Move the scoreboard_HiveTracker file into the /layout folder of TSH where all the other scoreboard files are located.
- Go to OBS and add a new Browser source.
- Select the /layout/scoreboard_HiveTracker/index.html as the local file and set its width and height to 1920 and 1080 respectively.
- To use the one without the character images, select the /layout/scoreboard_HiveTracker/noCharacter.html as the local file and set its width and height to 1920 and 1080 respectively.
