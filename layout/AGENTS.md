...
- `scoreboard_HiveTracker` is a direct copy of JinTracker with an added `hive.css` and decorative portrait-frame SVGs in `index.html`. Preserve the shared colors, base controller, typography, logo, and placement; all hive accents use the existing P1/P2 CSS variables. `hive-hex.svg` is the single reusable rounded-hex silhouette for portrait surfaces, wells, and shadows. Portrait hexes are always present: `index.js` uses a selected stock icon first, then `online_avatar`, then local `avatar`, and finally `player-placeholder.svg`. The name lane reserves 72px adjacent to each hex so `FitText` measures only visible name space. See root `AGENTS.md` for inherited caveats and verification scope.
