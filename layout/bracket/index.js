LoadEverything().then(() => {
    ...
    Update = async (event) => {
        ...
        if (
            ...
            let biggestRound = Math.max.apply(
                ...
                Object.values(bracket).map((r) => {
                    const setMap = Object.values(r.sets).map((s) => {
                        return s.playerId[0] == -1 || s.playerId[1] == -1
                            ? 0
                            : 1;
                    });
            ...
            if (window.LOSERS_ONLY)
                containerSize = $(".losers_container").height();

            while (
                biggestRound *
                    (2 * parseInt($(":root").css("--player-height")) + 4) >
                containerSize - 20
            ) {
                size -= 1;
                $(":root").css("--player-height", size);
            ...
            if (
                ...
                progressionsIn !=
                    _.get(oldData, "bracket.bracket.progressionsIn") ||
                progressionsOut !=
                    _.get(oldData, "bracket.bracket.progressionsOut") ||
                ...
                let winnersRounds = Object.fromEntries(
                    Object.entries(bracket).filter(
                        ([round]) => parseInt(round) > 0,
                    ),
                );

                Object.entries(winnersRounds).forEach(
                    ([roundKey, round], r) => {
              ...
              `;
                ...
                if (!window.WINNERS_ONLY) {
                    ...
                    Object.entries(losersRounds).forEach(
                        ([roundKey, round], r) => {
                            html += `<div class="round round_${roundKey}">`;
                            html += `<div class="round_name"></div>`;
                            Object.values(round.sets).forEach((slot, i) => {
                ...
                Object.entries(bracket).forEach(function (
                    [roundKey, round],
                    r,
                ) {
                    ...
                    Object.values(round.sets).forEach(
                        function (slot, i) {
                            ...
                            if (
                                ...
                                if (!slotElement || !slotElement.offset())
                                    return;
                ...
                Object.entries(bracket).forEach(function (
                    ...
                    Object.values(round.sets).forEach((set, setIndex) => {
                        ...
                        anim.add(
                            AnimateLine(
                                $(`.line_in_r_${roundKey}.s_${setIndex + 1}`),
                            ),
                            0,
                        );
                        ...
                        anim.add(
                            AnimateLine(
                                $(`.line_r_${roundKey}.s_${setIndex + 1}`),
                            ),
                            0.9,
                        );
                        anim.add(
                            AnimateLine(
                                $(`.line_out_r_${roundKey}.s_${setIndex + 1}`),
                            ),
                            1.4,
                        );
